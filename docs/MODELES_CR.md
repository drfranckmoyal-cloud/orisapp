# Modèles de compte rendu — cadrage

*Ouvert le 21 septembre 2026 à partir du dossier Word « Templates_CR_Franck_Moyal_WORD »
(Bureau de Franck, non recopié ici). Dire « on reprend les modèles de CR » suffit.*

## Règles figées (README des modèles)

- En-tête complet en page 1 ; hiérarchie identité > titre > bloc patient > rubriques.
- Documentation clinique (photos légendées) toujours sur une nouvelle page, et **rien de
  clinique après les photos**.
- Une rubrique sans contenu est **supprimée**, jamais laissée vide ni notée N/A — déjà le
  comportement d'Oris.
- RPPS et adresse du cabinet : à renseigner dans Paramètres, imprimés partout.

## Décisions du 21 septembre 2026

| # | Question | Réponse de Franck |
|---|---|---|
| 1 | Antécédents, traitements, allergies | Dans « Points d'attention » |
| 2 | Radios, examens complémentaires | Dans « Examen clinique » |
| 3 | Options discutées / décision retenue | Une seule rubrique « Proposition thérapeutique », **sans statut** : l'acceptation fait foi par le devis |
| 4 | Informations données au patient | Rubrique gardée |
| 5 | Actes réalisés pendant une consultation | Rubrique ajoutée au compte rendu de consultation |
| 6–7 | Consultation ou suivi | On oublie le type « suivi » (voir point ouvert B) |
| 8 | Bloc patient du courrier d'adressage | « Destinataire » au lieu de « Adressé(e) par » |
| 9 | Plan de traitement | Pas de modèle : à construire ensemble |
| 10 | Format | Modifiable dans Oris, sortie PDF. Pas de Word |
| 11 | Style | Plutôt télégraphique — à caler ensemble sur exemples |

## Rubriques retenues

**Compte rendu de consultation** : Motif · Examen clinique (dont radios) · Diagnostic /
analyse · Proposition thérapeutique · Informations données au patient · Actes réalisés ·
Suite de la prise en charge · Points d'attention / coordination (dont antécédents).

**Compte rendu opératoire** : Indication · Situation pré-opératoire · Intervention réalisée
· Protocole / éléments techniques · Résultat immédiat · Suites et consignes · Coordination
/ prochaine étape.

**Courrier d'adressage** : bloc Destinataire · Motif de l'adressage · Contexte clinique ·
Éléments pertinents · Examens disponibles / pièces jointes · Demande / objectifs · Points
d'attention / précautions · Coordination / retour souhaité · « Bien confraternellement ».

## Points tranchés le 21 septembre (suite)

- **A.** La proposition thérapeutique ne liste que ce qui est proposé ; une option écartée
  n'y figure pas (elle reste dans le dossier clinique).
- **B.** Pas de modèle « suivi » : un contrôle est une consultation.
- **C.** Formule d'appel « Cher confrère, » sous le bloc Destinataire.
- **D.** Ordre : Motif · Examen clinique · Diagnostic / analyse · Proposition thérapeutique
  · Informations données au patient · Actes réalisés · Suite de la prise en charge ·
  Points d'attention / coordination.
- **E.** Style **rédigé** (révisé le 21/09 : d'abord télégraphique, puis Franck
  préfère un compte rendu rédigé, en phrases). Les éléments d'une rubrique forment un
  paragraphe lisible plutôt qu'une ligne par fait ; « Rapporté par le patient /
  Constaté » est dit dans la phrase quand il compte. Numéros de dent et négations
  jamais raccourcis. Calage à faire sur exemples réels.

## Avancement

- **21/09 — Compte rendu de consultation** : rubriques et ordre du modèle, phrases avec
  les mots dits, validateur `negation_unclear`.
- **21/09 — PDF au modèle** : en-tête, titres (Paramètres), bloc patient avec « Adressé(e)
  par » / « Destinataire », rubriques vertes, paragraphes, page « Documentation clinique »
  avec photos légendées.
- **21/09 — Compte rendu opératoire** dans les rubriques du modèle 02 ; **courrier
  d'adressage** (modèle 04) à la demande.
- Reste : rédaction plus liée (phrases fusionnées par un modèle de langue, sous contrôle
  du validateur), trois styles (point H), plan de traitement (point F), photos HEIC.

## Points ouverts

- **F. Plan de traitement** : modèle à écrire ensemble.
- **G. Le déroulé pendant l'écoute** : niveau simple d'abord (liste fixe des rubriques),
  choix « consultation » ou « acte » avant l'écoute. Le niveau vivant (rubriques cochées
  quand elles sont abordées) viendra après essai.
- **H. Trois styles de rédaction au choix** (idée de Franck, 21/09, à garder) :
  **télégraphique**, **rédigé** (le style par défaut, point E), **exhaustif** — ce
  dernier reprend une bonne partie de l'échange avec le patient. Vigilance pour
  l'exhaustif : il reste bâti sur les faits du dossier clinique et ne cite que des
  paroles réellement entendues, attribuées à qui les a dites.
